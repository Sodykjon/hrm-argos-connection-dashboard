...
export function VakansiyaMap({
  ...
  useEffect(() => {
    ...
    const tooltipFormatter = (p: {
      name: string;
      data?: { share: number; vacant: number; filled: number; stavka: number };
    }) => {
      const label = regionLabel(p.name, lang);
      const d = p.data;
      if (!d || d.share === undefined) return label;
      return `<b>${label}</b><br/>${S.vakansiya.col.rate}: <b>${fmtPct(
        d.share,
      )}</b><br/>${S.vakansiya.col.vacant}: ${fmtInt(d.vacant)}<br/>${
        S.vakansiya.col.filled
      }: ${fmtInt(d.filled)}<br/>${S.vakansiya.col.stavka}: ${fmtInt(d.stavka)}`;
    };
